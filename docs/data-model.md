...
- **The default asks codex for a detailed reasoning summary (`-c model_reasoning_summary=detailed`).** Plan-level review is the case where a bare "no issues" is uninspectable — the *why* is the value. This flag dials codex's reasoning summary up from its `auto` default to `detailed`, so the run shows whether it actually examined a risky cross-unit seam versus skimmed past it, which is also what makes the re-review-to-convergence "should-fix" call judgeable. Caveat: it is a model-generated **summary**, not codex's raw chain-of-thought (that stays hidden) — treat it as a faithful-ish gloss, not ground truth. The relay stays **findings-first**: `/jidoka:plan-review-prompt` surfaces the findings as the product and the reasoning selectively (borderline findings / on request), never a default dump (see that skill, step 6). It applies to any reasoning-capable model codex runs; a non-reasoning model just ignores it. Cost: a few more reasoning-summary output tokens per review — negligible at plan-close cadence.
