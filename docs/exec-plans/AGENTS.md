...
- **`completed/index.md`** (plans) — `grep -rhm1 '^# ' --include='overview.md' docs/exec-plans/completed/ | sed 's/^# /- /' | sort`. The reversed-decision entries (loose `*.md` archived from `wiki/`) are listed by hand under their own heading — few, and frozen.
...
A **reversed decision** archived here from the repo's `wiki/` instead carries the superseded form — `> STATUS: superseded · <YYYY-MM> · replaced-by <what>, kept as record` — and keeps its topic name (`rust-runtime.md`) rather than a plan id.
