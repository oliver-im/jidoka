...
- On approval — it covers **only this close-out** — **archive + open a PR** per the git workflow above (`git mv` to `completed/`, stamp, commit on the plan branch, push, `gh pr create`; the PR merges on GitHub, then update `main` and remove the worktree). Don't archive before sign-off, even if `## Plan-level review` is empty.
