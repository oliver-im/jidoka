...
- Tests (`ts/__tests__/config.test.ts` + schema cases): accepts a slash string; accepts a template object; template defaults `mode: "print"`; rejects empty `run`, bad `mode`, and a non-`/` bare string; a project `.jidoka.json` setting any review array is ignored.
