...
Tasks:
- Add a dated decision section "Generalize review steps to tool-agnostic templates" that captures the three settled decisions: (1) relax `reviewCommandSchema` to accept a slash command **or** a bash template; (2) planview authors its own plan-level review prompt rather than vendoring codex's (codex's is diff/code-shaped, generic, drifts on upstream change, and redistributes someone else's text — with `codex exec`, codex supplies only the *model*, planview the *prompt*); (3) `print` default / `exec` opt-in, with rationale (the human checkpoint at the last lifecycle gate + the cost profile of a full-diff adversarial review; the Bash bypass is legitimate because `disable-model-invocation` blocks only the SlashCommand route, not Bash).
