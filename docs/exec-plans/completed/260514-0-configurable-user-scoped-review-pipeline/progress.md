...
- **05-docs-sweep** — `docs/data-model.md` gained a full Review-pipelines section with config + resolved shapes, substitution rule, slash/bash inference, validation rules, and glossary entries (Tool, ReviewStep, ReviewPipeline, Plan-level review); `docs/agent-guide.md` and `skills/jidoka/SKILL.md` drop the "parser hard-codes" claim; `notes/plan/AGENTS.md` resume protocol adds the last-Unit plan-level review + archive guidance; `README.md` documents the two new config keys + points to `jidoka:configure`.
