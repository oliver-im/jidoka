...
- `docs/agent-guide.md`:
  ...
  - Replace with a brief note that review steps come from `~/.claude/plugins/jidoka/config.json` now and the skill produces no review info.
...
- `README.md`: short paragraph naming the new config keys (`tools`, `review_pipelines`) and pointing to `jidoka:configure`.
