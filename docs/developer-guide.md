...
| `plan_review` | `ReviewStep[]` | `[{ run: "codex exec -s read-only -c model_reasoning_summary=detailed \"{focus}\" < /dev/null", mode: "exec" }]` | **no** | Plan-level review steps (ships on — a `codex exec` template, agent-run via `/jidoka:plan-review-prompt`; set `[]` to disable), same `ReviewStep` shape and global-only boundary. `-c model_reasoning_summary=detailed` asks codex for its fullest reasoning summary (a summary, not raw chain-of-thought) so the review surfaces *why* it flagged/cleared a seam, not just a verdict. The trailing `< /dev/null` is the stdin hang-guard so an unattended `exec` run can't block on an open stdin pipe (`codex exec` appends piped stdin as a `<stdin>` block); see [data-model.md](data-model.md#command-semantics--invocation). |
