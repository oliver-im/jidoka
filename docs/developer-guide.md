...
| `unit_review` | `ReviewStep[]` | `[{ run: "claude -p '/code-review {diff_range}' < /dev/null", mode: "exec" }]` | **no** | Per-unit review steps, same `ReviewStep` shape and global-only boundary. The built-in `/code-review` is `disable-model-invocation`, so the default reaches it as a `claude -p` template (agent-run via Bash) rather than a bare slash command; `{diff_range}` is required because an unranged run reviews the wrong scope and does not fail closed on an empty diff (see [data-model.md](data-model.md#command-semantics--invocation)). |
| `plan_review` | `ReviewStep[]` | `[{ run: "codex exec -s read-only -c model_reasoning_summary=detailed \"{focus}\" < /dev/null", mode: "exec" }]` | **no** | Plan-level review steps (ships on — a `codex exec` template, agent-run via `/jidoka:plan-review-prompt`; set `[]` to disable), same `ReviewStep` shape and global-only boundary. `-c model_reasoning_summary=detailed` adds codex's reasoning narration (a summary, not raw chain-of-thought) to the saved transcript for on-demand inspection; the *why flagged/cleared* itself is required in the reviewer's output — findings + per-focus-target dispositions. The trailing `< /dev/null` is the stdin hang-guard so an unattended `exec` run can't block on an open stdin pipe (`codex exec` appends piped stdin as a `<stdin>` block); see [data-model.md](data-model.md#command-semantics--invocation). |
