...
planview is a Claude Code plugin that visualizes multi-agent task decomposition as Mermaid diagrams in the browser. It has two components:

- **Skill** (forked subagent) — LLM analyzes a task, produces topology JSON, saves to `/tmp/planview-{session_id}.json`
- **Renderer** (compiled Rust binary) — deterministic CLI that validates JSON, generates Mermaid graphs + HTML, opens browser. Never calls the LLM.

The contract between them: `echo '<topology JSON>' | planview` → stdout is the HTML file path, exit 0 on success.
...
- **Hook integration:** PreToolUse hook on ExitPlanMode renders plan+diagram before user approval. If no topology exists, hook denies with instruction to run `/planview` first (max 3 attempts, then falls through).
...
- **Runtime:** Rust (cargo build, cargo test, single binary via `cargo build --release`)
