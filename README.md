...
After each unit, the default action is to run a review cycle by another agent, and then stop until explicit human approval. This gives the reviewer the chance to see 1) the implementation overview 2) the review agent findings 3) whether to compact or reset the context before the next implementation starts. The one exception is the last unit: instead of stopping before the plan-level review, the session rolls straight through it — review, fixes, re-review — and stops with the full picture surfaced; the approval that follows covers only the close-out — archiving the plan, and nothing more. Publishing is a separate, explicitly-requested step: a close-out go-ahead is a finish-the-work instruction, not permission to push a branch or merge it.

For the review process, jidoka has the ability to insert custom bash commands (to run another agent like codex or opencode), or claude plugins to be executed before implementation (`pre_review`), after implementation of each unit (`unit_review`), and after implementation of the entire plan (`plan_review`). Prompt injection is **plan-level only**: the `/jidoka:plan-review-prompt` composer fills a `plan_review` template's `{focus}` with jidoka's own review prompt plus the cross-unit targets. Unit review gets no such steering — `/code-review` takes no focus argument, so per-unit review focus belongs in the unit body prose, where the triager reads it. The default settings use the built-in `/code-review` for each unit and codex at plan-close, and again is heavily geared towards the author's taste. Note that `/code-review` sets `disable-model-invocation`, so a resuming agent can't invoke it as a slash command; the shipped `unit_review` reaches it as `claude -p '/code-review {diff_range}' < /dev/null` in `exec` mode, which keeps the unit loop flowing (the `< /dev/null` is a stdin hang-guard — don't drop it when hand-editing a step, or an unattended run can block forever on the open pipe). Set it back to a bare `"/code-review"` if you'd rather stop for a human at every unit — noting that the bare command carries no range, so on a unit branch with no upstream it widens to the whole plan so far. By default jidoka also renders a *re-review-to-convergence* note into each review section (the `review_reconverge` toggle, on by default): after a review flags a finding at or above the reviewer's own middle "should-fix" severity, re-run that review once on the post-fix diff — the fixes are otherwise unreviewed code. Set `review_reconverge: false` in the global config (`~/.claude/plugins/jidoka/config.json`) to omit it — like the review arrays, it is global-config-only and can't be set per-repo in `.jidoka.json`.
