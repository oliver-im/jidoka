...
| `plan_review` | `[]` | — | Review steps to run after the last Unit's review and commit, against the cumulative committed diff. Opt-in; the recommended vehicle is the `/jidoka:plan-review-prompt` composer driving a `codex exec` template or `/codex:adversarial-review` (see below). Rendered as `## Plan-level review` in `progress.md`. |
