...
For the review process, jidoka has the ability to insert custom bash commands (to run another agent like codex or opencode), or claude plugins to be executed before implementation (`pre_review`), after implementation of each unit (`unit_review`), and after implementation of the entire plan (`plan_review`). The project has a baseline prompt, and the skill appends custom prompts to steer the review agent. The default settings uses `/code-review` plugin and codex for review, and again is heavily geared towards the author's taste. By default jidoka also renders a *re-review-to-convergence* note into each review section (the `review_reconverge` toggle, on by default): after a review flags a finding at or above the reviewer's own middle "should-fix" severity, re-run that review once on the post-fix diff — the fixes are otherwise unreviewed code. Set `review_reconverge: false` in the global config (`~/.claude/plugins/jidoka/config.json`) to omit it — like the review arrays, it is global-config-only and can't be set per-repo in `.jidoka.json`.
