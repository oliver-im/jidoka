...
5. **⚠️ Close stdin when the prompt is an argument (the unattended-hang guard).** `codex exec [PROMPT]` reads stdin and appends it as a `<stdin>` block *whenever stdin is a pipe* — per `codex exec --help`: "If stdin is piped and a prompt is also provided, stdin is appended as a `<stdin>` block." In an unattended / non-interactive `exec` run (the Bash tool, a backgrounded shell — stdin is an open pipe that never sends EOF), codex therefore **blocks forever**, printing only `Reading additional input from stdin...` and never returning (a real ~5-hour plan-review hang was observed here). A TTY foreground run sees no piped stdin and just uses the arg — which is exactly why this slips past interactive testing. So: **whenever you deliver `{focus}` as an argument** — a quoted arg, a `"$(cat <tmpfile>)"`, or a command-substitution heredoc `"$(cat <<'EOF' … EOF\n)"` — **append `< /dev/null` to the final command**, e.g. `codex exec -s read-only "<focus>" < /dev/null`. This closes stdin so codex just uses the arg; it is a no-op on a foreground TTY, so keep it in `print` output too. **Exception:** if you delivered the prompt via the `codex exec -` **stdin-prompt** form (or a `git diff … | codex exec` feed), the prompt *is* stdin and gets a clean EOF — do **not** add `< /dev/null` there (it would starve the prompt). Note `< /dev/null` is an empty-stdin guard, *not* a diff feed — the delivery decision in step 3 is unchanged (the reviewer still fetches the diff itself).
6. Then honor `mode`:
   ...
   - **`exec`**: run the substituted command via the **Bash** tool, capture its output, and surface the reviewer's findings (don't editorialize — relay them, noting the base/range you used). The Bash tool's stdin is not a TTY, so the arg form **must** carry `< /dev/null` (step 5) or this call hangs indefinitely. If the tool needs auth/setup (e.g. `codex login`) and isn't ready, say so and fall back to presenting the command for the operator.
     - **Keep the transcript addressable.** Redirect the run's output to a file (`> <tmpfile> 2>&1`; a backgrounded run's task output file also works) rather than letting it stream inline — a real review transcript far exceeds any inline result cap, and both the verification below and any later "go deeper" request need to grep and re-read slices of it.
     - **Verify the requested reasoning level took effect.** The transcript opens with a header echoing the run's *effective* settings (`reasoning effort: …` / `reasoning summaries: …`). If the template asked for a summary level (the default `-c model_reasoning_summary=detailed`) but the header reports another (`none`), the request was silently dropped — a stale config, a codex regression, a model without summary support. Put one line in your relay (⚠️ requested `detailed` reasoning summaries; header says `none`) instead of letting the degradation pass unremarked — the findings are unaffected, but a knob the user believes is on is off.
     - **Findings + dispositions are the product; the transcript is the source.** Relay the reviewer's output block — the findings **and** its `### Focus dispositions` section (the per-target *why clean*, which the prompt requires with evidence citations) — and name the transcript file's path so the user can go deeper. Don't paste the transcript's reasoning narration wholesale (it buries the actionable signal). When a disposition or a borderline finding needs checking, verify against the transcript's **exec blocks** — the commands the reviewer actually ran and their raw output (`grep -n '^exec$' <tmpfile>` locates them in current codex output): they are ground truth of what was inspected. The header lines and block markers are codex's output format, not a contract — if a marker doesn't match (a codex update, a different vehicle), fall back to reading the transcript's shape directly, and if you can't verify, say *verification unavailable* rather than concluding nothing ran. The interleaved reasoning narration is a model-generated **summary**, not raw chain-of-thought — read it on demand for intent context ("what was it examining here"), and cite it as supporting evidence, not as proof of what the model "really did".
